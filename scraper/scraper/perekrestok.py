...
class PerekrestokScraper(BaseScraper):
    ...
    async def scrape_category(self, category_url: str, max_products: int = 80) -> List[Dict]:
        ...
        try:
            print(f"🌐 [Перекрёсток] Открываем: {category_url}")
            await page.goto(category_url, wait_until="networkidle", timeout=90000)
            await self.random_delay(6, 11)

            items = await page.query_selector_all('article, div[data-testid*="product"], div[class*="ProductCard"]')
            ...
            print(f"✅ [Перекрёсток] Спарсено: {len(products)}")
