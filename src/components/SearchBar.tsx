...
const FALLBACK_SUGGESTIONS = ["молоко 3.2%", "куриная грудка", "яйца C1", "бананы", "сыр российский", "масло сливочное"];
...
export function SearchBar({ variant = "hero", defaultValue = "" }: Props) {
  ...
  return (
    <div ref={wrapRef} className="relative w-full">
      ...
      >
        <Search className={isHero ? "ml-3 h-6 w-6 text-muted-foreground" : "h-4 w-4 text-muted-foreground"} />
        <input
        ...
        {value && (
          <button type="button" onClick={() => { setValue(""); inputRef.current?.focus(); }}
            className="rounded-full p-1.5 text-muted-foreground hover:bg-muted" aria-label="Очистить">
            <X className="h-4 w-4" />
          </button>
        )}
        {hasVoice && isHero && (
          <button type="button" onClick={startVoice} aria-label="Голосовой поиск"
          ...
          {showAutocomplete ? (
            <ul className="max-h-96 overflow-y-auto py-1">
              {autocompleteData!.items.map((item) => (
                <li key={item.id}>
                  <button type="button"
                    onClick={() => { setValue(""); setOpen(false); navigate({ to: "/product/$slug", params: { slug: item.slug } }); }}
            ...
            <div className="px-4 py-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                {history.map((h) => (
                  <button key={h} type="button" onClick={() => { setValue(h); submit(h); }}
