        ...
        <div className="mb-6 flex items-center justify-between">
          ...
          {items.length > 0 && (
            <div className="flex gap-2">
              ...
              <button onClick={() => { clearAll(); hapticLight(); toast.success("Список очищен"); }}
                ...
                <div className="border-b border-border px-4 py-3">
                  ...
                  {saving > 0 && (
                    <p className="mt-0.5 text-xs text-muted-foreground">
                      Экономия до <span className="font-semibold text-success">{formatRub(saving)}</span> если выбрать лучший магазин
                    </p>
                  )}
                </div>
                      ...
                      <div key={s.store_id}
                        ...
                        <div className="flex items-center gap-2.5">
                          <span className="flex h-8 w-8 items-center justify-center rounded-lg text-xs font-bold"
                            style={{ backgroundColor: `${s.brand_color}20`, color: s.brand_color ?? undefined }}>
                            {i + 1}
                          </span>
                          <div>
                            <p className="text-sm font-medium">{s.store_name}</p>
                            <p className="text-xs text-muted-foreground">{s.available} из {items.length} товаров</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className={`font-display text-lg font-bold ${isBest ? "text-success" : ""}`}>
                            {formatRub(s.total)}
                          </p>
                          {diff > 0 && (
                    ...
                    <div className="flex items-center gap-3 p-3">
                      ...
                      <Link to="/product/$slug" params={{ slug: product.slug }}>
                        {product.image_url
                          ...
                          : <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl bg-muted text-2xl">🛒</div>}
                      </Link>

                      {/* Инфо */}
                      <div className="min-w-0 flex-1">
                        <Link to="/product/$slug" params={{ slug: product.slug }}>
                          <p className="line-clamp-1 text-sm font-medium hover:text-primary">{product.name}</p>
                        </Link>
                        ...
                        <p className="mt-0.5 text-sm font-semibold text-success">
                          ...
                          {qty > 1 && <span className="ml-1 text-xs text-muted-foreground">× {qty} = {formatRub((product.best_price ?? 0) * qty)}</span>}
                        </p>
                      </div>

                      {/* Кол-во + удалить */}
                      <div className="flex items-center gap-1">
                        <button onClick={() => { updateQty(product.id, qty - 1); hapticLight(); }}
                        ...
                        <button onClick={() => { removeItem(product.id); hapticLight(); }}
