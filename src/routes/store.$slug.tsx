...
const getStoreData = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    function enrich(rows: any[]) {
      return rows.map((p) => {
        const offers = (allOffers || []).filter((o) => o.product_id === p.id).map((o) => ({
          store_id: o.store_id, store_slug: (o.stores as any)?.slug ?? "",
          store_name: (o.stores as any)?.name ?? "", brand_color: (o.stores as any)?.brand_color ?? null,
          price: Number(o.price), old_price: o.old_price ? Number(o.old_price) : null,
          is_promo: o.is_promo, store_product_url: o.store_product_url,
        }));
        ...
        return {
          ...
          offers, best_price: prices.length ? Math.min(...prices) : null,
...
export const Route = createFileRoute("/store/$slug")({
  ...
  loader: ({ context, deps }) =>
    context.queryClient.ensureQueryData(
      queryOptions({ queryKey: ["store", deps.slug], queryFn: () => getStoreData({ data: { slug: deps.slug } }) })
    ),
  head: ({ params }) => ({ meta: [{ title: `${params.slug} — Ценомер` }] }),
  component: () => <Suspense fallback={<div className="min-h-screen bg-background"><SiteHeader compact /><div className="p-4"><ProductGridSkeleton /></div></div>}><StorePage /></Suspense>,
});
      ...
      <main className="mx-auto max-w-7xl px-4 py-8">
        ...
        <div className="mb-8 overflow-hidden rounded-2xl border border-border bg-card shadow-soft">
          ...
          <div className="flex items-center gap-5 p-6">
            <div className="grid h-16 w-16 shrink-0 place-items-center rounded-2xl text-2xl font-bold text-white shadow-soft"
            ...
            <Link to="/search" search={{ q: store.name }}
        ...
        {promoProducts.length > 0 && (
          <section className="mb-10">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h2 className="font-display text-2xl font-bold tracking-tight">Акции в {store.name}</h2>
            </div>
        ...
        {recentProducts.length > 0 && (
          <section>
            <h2 className="mb-4 font-display text-2xl font-bold tracking-tight">Недавно обновлённые</h2>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 lg:grid-cols-4">
