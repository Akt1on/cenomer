...
function FavoritesPage() {
  ...
  async function loadFavorites() {
    ...
    try {
      ...
      const [{ products }, { data: alerts }] = await Promise.all([
        ...
        supabase.from("price_alerts").select("product_id")
  ...
  async function removeFavorite(productId: string) {
    ...
    if (error) { toast.error("Не удалось удалить"); loadFavorites(); }
  ...
  async function toggleTrack(product: ProductWithOffers) {
    ...
    if (isTracked) {
      hapticLight();
      const { error } = await supabase.from("price_alerts").delete()
        .eq("user_id", user!.id).eq("product_id", product.id);
      if (error) { setTracked((prev) => ({ ...prev, [product.id]: true })); toast.error("Ошибка"); }
      else toast.success("Отслеживание выключено");
    } else {
      hapticSuccess();
      const { error } = await supabase.from("price_alerts").upsert(
        { user_id: user!.id, product_id: product.id, target_price: product.best_price, is_active: true },
        { onConflict: "user_id,product_id" },
      );
      if (error) { setTracked((prev) => ({ ...prev, [product.id]: false })); toast.error("Ошибка"); }
      ...
      <main className="mx-auto max-w-7xl px-4 py-8">
        ...
        {loading ? <ProductGridSkeleton count={8} /> : items.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-border bg-card p-12 text-center">
            ...
            <Link to="/" className="mt-5 inline-block rounded-xl bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground shadow-soft transition hover:opacity-90">
              Найти товары
            </Link>
          </div>
        ) : (
                ...
                <motion.div key={p.id} layout
                  ...
                  <div className="absolute right-2 top-2 flex flex-col gap-1">
                    <button onClick={() => toggleTrack(p)}
                      ...
                      {tracked[p.id] ? <Bell className="h-3.5 w-3.5" /> : <BellOff className="h-3.5 w-3.5" />}
                    </button>
                    <button onClick={() => removeFavorite(p.id)} title="Удалить"
